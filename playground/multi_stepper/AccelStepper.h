...
/// \li Supports alternate stepping functions to enable support of AFMotor
/// (https://github.com/adafruit/Adafruit-Motor-Shield-library) \li Supports stepper drivers such as the Sparkfun
/// EasyDriver (based on 3967 driver chip) \li Very slow speeds are supported \li Extensive API \li Subclass support
///
...
/// See https://github.com/adafruit/Adafruit_Motor_Shield_V2_Library for examples that work with Adafruit Motor Shield
/// V2.
///
/// \par Donations
///
...
/// value="_donations" /> <input type="hidden" name="business" value="mikem@airspayce.com" /> <input type="hidden"
/// name="lc" value="AU" /> <input type="hidden" name="item_name" value="Airspayce" /> <input type="hidden"
/// name="item_number" value="AccelStepper" /> <input type="hidden" name="currency_code" value="USD" /> <input
/// type="hidden" name="bn" value="PP-DonationsBF:btn_donateCC_LG.gif:NonHosted" /> <input type="image" alt="PayPal —
/// The safer, easier way to pay online." name="submit"
/// src="https://www.paypalobjects.com/en_AU/i/btn/btn_donateCC_LG.gif" /> <img alt=""
...
/// \version 1.34  Added setPinsInverted(bool pin1Invert, bool pin2Invert, bool pin3Invert, bool pin4Invert, bool
/// enableInvert)
...
/// \version 1.41  Fixed an error in the acceleration calculations, resulting in acceleration of haldf the intended
/// value \version 1.42  Improved support for FULL3WIRE and HALF3WIRE output pins. These changes were in Yuri's original
///                contribution but did not make it into production.<br>
/// \version 1.43  Added DualMotorShield example. Shows how to use AccelStepper to control 2 x 2 phase steppers using
/// the
...
///                Fixed a problem reported by gregor: when resetting the stepper motor position using
///                setCurrentPosition() the stepper speed is reset by setting _stepInterval to 0, but _speed is not
...
class AccelStepper {
 public:
  ...
  /// preferably in your main loop. Note that each call to run() will make at most one step, and then only when a step
  /// is due, based on the current speed and the time since the last step. \return true if the motor is still running to
  /// the target position.
  boolean run();
  ...
  /// Returns the direction of the stepper motor
  AccelStepper::Direction getDirection();
